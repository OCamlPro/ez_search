...
let pcre_dfa_exec ?(iflags = 0) ?flags ?(rex = def_rex) ?pat ?(pos = 0)
    ?callout ?(workspace = Array.make 20 0) ?len subj =
  let len = match len with
    | None -> String.length subj
    | Some len -> len
  in
  ...
  unsafe_pcre_dfa_exec
    iflags rex ~pos ~subj_start:0 ~subj ovector callout ~workspace ~len;
  ovector
...
let pcre_exec ?(iflags = 0) ?flags ?(rex = def_rex) ?pat ?(pos = 0)
  ...
  let iflags = match flags with Some flags -> rflags flags | _ -> iflags in
  let _, ovector = make_ovector rex in
  unsafe_pcre_exec iflags rex ~pos ~subj_start:0 ~len ~subj ovector callout;
  ovector

let exec ?iflags ?flags ?rex ?pat ?pos ?callout ?len subj =
  subj, pcre_exec ?iflags ?flags ?rex ?pat ?pos ?callout ?len subj
  ...
  let rec loop full_len trans_lsts cur_pos =
    if
      ...
      try
        ...
        unsafe_pcre_exec
          iflags rex ~pos:cur_pos ~subj_start:0 ~subj ~len
          ovector callout;
        false
      with Not_found -> true
    then
  ...
  let rec loop full_len subst_lst cur_pos =
    if
      ...
      try
        let len = String.length subj in
        unsafe_pcre_exec
          iflags rex ~pos:cur_pos ~subj_start:0 ~subj ~len ovector callout;
        false
      with Not_found -> true
    then
      let postfix_len = max (subj_len - cur_pos) 0 in
...
let replace_first ?(iflags = 0) ?flags ?(rex = def_rex) ?pat ?(pos = 0)
  ...
  try
    let len = String.length subj in
    unsafe_pcre_exec iflags rex ~pos ~subj_start:0 ~subj ~len ovector callout;
        ...
        if cnt = 0 then
          if prematch &&
             try
               let len = String.length subj in
              unsafe_pcre_exec
                flags rex ~pos ~subj_start:pos ~subj ~len ovector callout;
              true
            with Not_found -> false
          then
            let last = Array.unsafe_get ovector 1 in
              ...
              if last = pos then
                ...
                if len = 0 then "" :: strs
                  ...
                  try
                    let len = String.length subj in
                    unsafe_pcre_exec
                      (flags lor 0x0410) rex ~pos ~subj_start:pos ~subj ~len
                      ovector callout;
                    true
                  with Not_found -> false
                then
        ...
        if cnt = 0 then
          if prematch &&
            try
              let len = String.length subj in
              unsafe_pcre_exec
                iflags rex ~pos ~subj_start:pos ~subj ~len ovector callout;
               true
            with Not_found -> false
          then
            let first = Array.unsafe_get ovector 0 in
                ...
                if len = 0 then handle_subgroups (Delim "" :: strs)
                  ...
                  if
                    try
                      let len = String.length subj in
                      unsafe_pcre_exec
                        (iflags lor 0x0410) rex ~pos ~subj_start:pos ~subj ~len
                        ovector callout;
                      true
                    with Not_found -> false
                  then
