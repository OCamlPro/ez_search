...
external re_partial_match: regexp -> string -> int -> int array
     = "re_partial_match"
external re_search_forward: regexp -> string -> int -> int -> int array
     = "re_search_forward"
...
let search_forward re s ?len pos =
  let maxlen = String.length s in
  let len = match len with
    | None -> maxlen
    | Some len -> min len maxlen
  in
  let res = re_search_forward re s pos len in
  last_search_result := res;
