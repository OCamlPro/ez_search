...
CAMLprim value re_search_forward(value re, value str,
                                 value startpos, value endpos)
{
  unsigned char * starttxt = &Byte_u(str, 0);
  unsigned char * txt = &Byte_u(str, Long_val(startpos));
  unsigned char * endtxt = &Byte_u(str, Long_val(endpos));
  unsigned char * maxtxt = &Byte_u(str, caml_string_length(str));
  unsigned char * startchars;
  value res;

  if (txt < starttxt || txt > endtxt || endtxt > maxtxt)
